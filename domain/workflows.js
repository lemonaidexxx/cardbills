...
function billsList(entity,filters,page,sort) {
  return guard_(()=>{const db=load_();if(entity==='InstallmentOptions')return billsInstallmentOptions_(db,filters||{},page);if(entity==='LoanDashboard')return loanDashboard_(db,filters||{});if(entity==='OverviewCharts')return billsCharts_(db,filters||{});if(entity==='DuplicateReview')return billsDuplicates_(db,page);return billsListView_(db,review_(db),entity,filters,page,sort);},true);
}
