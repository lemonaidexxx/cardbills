...
function billsSave(entity,input,expectedToken,requestId) {
  if(['LoanBalance','LoanPaymentPrincipal','LoanPrincipalHistory','LoanDetails','TransactionInstallment','TransactionPrincipal','Loans','LoanSchedules','LoanPayment','ReverseLoanPayment','ShareBatch'].includes(entity))return billsLedgerAction_(entity,input,expectedToken,requestId);
  if(['LoanBalances','LoanPayments','LoanAllocations'].includes(entity))fail_('VALIDATION: Use Record payment or Reverse payment.');
...
function billsCharts_(db,filters){
 ...
 const currentMonth=filters.cardMonth||today_(db).slice(0,7);if(!/^\d{4}-\d{2}$/.test(currentMonth)||!dateValid_(currentMonth+'-01'))fail_('VALIDATION: Invalid card month.');
 const empty=()=>({months:months.map(month=>({month,amountMinor:0})),cards:[],types:[],currentCards:[],undatedCount:0}),currencies={};for(const currency of [...new Set(db.Accounts.map(a=>a.currency))])currencies[currency]=empty();
 const invalid=review_(db).some(i=>i.severity==='ERROR');if(invalid)return {endMonth:end,currentMonth,currencies,invalid};
 for(const t of db.Transactions){const charge=['PURCHASE','FEE','INTEREST','CASH_ADVANCE','INSTALLMENT'].includes(t.type),credit=['REFUND','REBATE'].includes(t.type);if(t.status!=='ACTIVE'||!charge&&!credit)continue;const group=currencies[t.currency]||(currencies[t.currency]=empty());if(!t.postingDate){if(charge)group.undatedCount++;continue;}if(t.postingDate.slice(0,7)===currentMonth){let card=group.currentCards.find(c=>c.id===(t.cardId||''));if(!card){card={id:t.cardId||'',amountMinor:0,chargesMinor:0,creditsMinor:0};group.currentCards.push(card);}card[charge?'chargesMinor':'creditsMinor']=addMinor_(card[charge?'chargesMinor':'creditsMinor'],t.amountMinor);card.amountMinor=addMinor_(card.amountMinor,t.amountMinor);}if(!charge)continue;const month=group.months.find(m=>m.month===t.postingDate.slice(0,7));if(!month)continue;month.amountMinor=addMinor_(month.amountMinor,t.amountMinor);for(const [key,id]of [['cards',t.cardId||''],['types',t.type]]){let item=group[key].find(v=>v.id===id);if(!item){item={id,amountMinor:0};group[key].push(item);}item.amountMinor=addMinor_(item.amountMinor,t.amountMinor);}}
...
function loanRecordIssues_(entity,r,db,add){
 ...
 if(entity==='LoanPayments'){if(Number(r.amountMinor)<=0)add('Payment must be positive');if(r.principalMinor!==undefined&&r.principalMinor!==''&&(!Number.isSafeInteger(Number(r.principalMinor))||r.principalMinor<0||r.principalMinor>r.amountMinor))add('Principal portion must be between zero and the payment amount');}
...
function billsReviewTransactions_(db,filters,page,sort){
 ...
 return {rows:rows.slice(p*40,p*40+40).map(r=>({...r,reviewIssues:byId.get(r.id),assignedShares:db.Shares.filter(s=>s.transactionId===r.id&&s.status==='ACTIVE').map(s=>({id:s.id,personId:s.personId,amountMinor:s.amountMinor}))})),total:rows.length,page:p,issues:[],otherIssues:issues.filter(i=>i.entity!=='Transactions'||!db.Transactions.some(t=>t.id===i.id))};
}
function billsInstallmentOptions_(db,f,page){
 const entity=f.mode==='plans'?'InstallmentPlans':'Transactions',q=String(f.q||'').trim().toLowerCase();
 const matches=db[entity].filter(r=>r.accountId===f.accountId&&r.currency===f.currency&&(!r.cardId||r.cardId===f.cardId)&&(entity==='Transactions'?r.status==='ACTIVE'&&r.type==='FINANCED_PRINCIPAL':r.status!=='ARCHIVED')&&(!f.principalId||r.originTransactionId===f.principalId)&&(!q||(String(r.description||r.reference||'')+' '+r.id).toLowerCase().includes(q))).sort((a,b)=>String(a.description||a.reference||'').localeCompare(String(b.description||b.reference||''))||a.id.localeCompare(b.id));
 const p=Math.max(0,Math.floor(Number(page)||0));return {rows:matches.slice(p*20,p*20+20).map(r=>decorate_(entity,r,db)),total:matches.length,page:p};
}
function billsLedgerAction_(entity,input,expectedToken,requestId){return guard_(()=>{
 ...
 if(['LoanBalance','LoanPrincipalHistory'].includes(entity)){
 const loan=db.Loans.find(r=>r.id===input.loanId);if(!loan||token_(loan)!==expectedToken)fail_('CONFLICT: Loan changed.');
 if(entity==='LoanPrincipalHistory'){if(!['true','false'].includes(input.complete))fail_('VALIDATION: Confirm whether payment history is complete.');changes.push({entity:'Loans',before:loan,after:prepare_('Loans',{principalHistoryComplete:input.complete},loan)});}
 else {if(!Number.isSafeInteger(input.balanceMinor)||input.balanceMinor<0||!dateValid_(input.throughDate)||input.throughDate>today_(db))fail_('VALIDATION: Enter a nonnegative lender balance and a valid date through today.');changes.push({entity:'LoanBalances',before:null,after:prepare_('LoanBalances',{loanId:loan.id,balanceMinor:input.balanceMinor,throughDate:input.throughDate,reference:input.reference||'',knownReversals:hash_(JSON.stringify(db.LoanPayments.filter(p=>p.loanId===loan.id&&p.status==='REVERSED'&&p.date<=input.throughDate).map(p=>p.id).sort()))},null)});}
 }
 if(entity==='LoanPaymentPrincipal'){const payment=db.LoanPayments.find(p=>p.id===input.id);if(!payment||payment.status!=='CONFIRMED'||token_(payment)!==expectedToken)fail_('CONFLICT: Payment changed.');const principal=input.principalMinor??'';if(principal!==''&&(!Number.isSafeInteger(principal)||principal<0||principal>payment.amountMinor))fail_('VALIDATION: Principal portion must be between zero and the payment amount.');changes.push({entity:'LoanPayments',before:payment,after:prepare_('LoanPayments',{principalMinor:principal},payment)});}
 ...
 if(!plan||token_(plan)!==input.planToken||!origin||token_(origin)!==input.principalToken||plan.originTransactionId!==origin.id)fail_('CONFLICT: Plan or principal changed. Select it again.');
 if(plan.status==='ARCHIVED'||origin.status!=='ACTIVE'||origin.type!=='FINANCED_PRINCIPAL'||after.type!=='INSTALLMENT'||after.installmentPlanId!==plan.id||plan.accountId!==after.accountId||plan.currency!==after.currency||(plan.cardId&&plan.cardId!==after.cardId)||origin.accountId!==after.accountId||origin.currency!==after.currency||(origin.cardId&&origin.cardId!==after.cardId))fail_('VALIDATION: Choose a compatible plan and principal.');
 const n=Number(after.installmentNumber);if(!Number.isInteger(n)||n<1||n>Number(plan.count))fail_('VALIDATION: Payment sequence must be within the plan.');
 if(db.Transactions.some(t=>t.id!==after.id&&t.status==='ACTIVE'&&t.type==='INSTALLMENT'&&t.installmentPlanId===plan.id&&Number(t.installmentNumber)===n))fail_('CONFLICT: Payment sequence already linked.');
 changes.push({entity:'Transactions',before,after});
 }
 if(entity==='TransactionPrincipal'){
 const before=db.Transactions.find(t=>t.id===input.transaction?.id);if(!before||token_(before)!==expectedToken)fail_('CONFLICT: Transaction changed.');
 const after=prepare_('Transactions',input.transaction,before);if(after.type!=='FINANCED_PRINCIPAL'||after.status!=='ACTIVE')fail_('VALIDATION: Choose an active financed principal.');
 if(after.installmentPlanId!==before.installmentPlanId||after.installmentNumber!==before.installmentNumber)fail_('VALIDATION: Principal links belong to the plan, not its payment sequence.');
 const plan=db.InstallmentPlans.find(p=>p.id===input.planId);if(!plan||token_(plan)!==input.planToken)fail_('CONFLICT: Installment plan changed.');
 if(plan.status==='ARCHIVED'||plan.accountId!==after.accountId||plan.currency!==after.currency||(plan.cardId&&plan.cardId!==after.cardId))fail_('VALIDATION: Choose a compatible plan.');
 if(plan.originTransactionId&&plan.originTransactionId!==after.id&&input.replacePrincipalId!==plan.originTransactionId)fail_('CONFLICT: Confirm replacement of the existing principal.');
 changes.push({entity:'Transactions',before,after},{entity:'InstallmentPlans',before:plan,after:prepare_('InstallmentPlans',{originTransactionId:after.id},plan)});
 }
...
function loanPrincipal_(db,loan){
 const balance=db.LoanBalances.filter(b=>b.loanId===loan.id).sort((a,b)=>b.throughDate.localeCompare(a.throughDate)||b.createdAt.localeCompare(a.createdAt)||b.id.localeCompare(a.id))[0];
 const base=balance?Number(balance.balanceMinor):loan.principalMinor!==''&&loan.principalMinor!==undefined?Number(loan.principalMinor):null,throughDate=balance?.throughDate||'',basis=balance?'Lender balance':'Original principal';
 const result={basis,throughDate,baseMinor:base,remainingMinor:null,complete:false,missingCount:0,status:'Balance needed'};
 if(base===null)return result;if(!balance&&loan.principalHistoryComplete!=='true')return {...result,status:'Confirm complete payment history'};
 if(balance&&balance.knownReversals!==hash_(JSON.stringify(db.LoanPayments.filter(p=>p.loanId===loan.id&&p.status==='REVERSED'&&p.date<=throughDate).map(p=>p.id).sort())))return {...result,status:'Reconciliation needed: payment reversed within lender balance period'};
 const payments=db.LoanPayments.filter(p=>p.loanId===loan.id&&p.status==='CONFIRMED'&&(!balance||p.date>throughDate)),missing=payments.filter(p=>p.principalMinor===''||p.principalMinor===undefined||p.principalMinor===null);
