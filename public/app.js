...
(() => {
  ...
  function renderOverview(){const o=state.boot.overview,c=$('content');
    ...
    o.upcoming.forEach(s=>{const item=button('',()=>showDetails('Statements',s),'statement-overview');const account=(state.boot.lookups.Accounts||[]).find(a=>a.id===s.accountId);const due=s.dueDate<state.boot.today?'Past due':s.dueDate===state.boot.today?'Due today':'Not yet due';append(item,el('strong','',account?.label.replace(/ · [^·]+$/,'')||lookup('Accounts',s.accountId)),el('strong','',money(s.remainingMinor,s.currency)),el('span','','Due '+s.dueDate),el('span',s.dueDate<state.boot.today?'badge overdue':'badge',due));upcoming.append(item);});c.append(upcoming);
const intro=el('section','overview-intro');append(intro,el('h2','','Your finances, clearly organized.'),el('p','subtle','Track spending, manage balances and review what needs your attention.'),button('Review transactions',()=>navigate('Review'),'secondary'));c.append(intro);
