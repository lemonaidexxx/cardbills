...
(() => {
  ...
  function renderUpcomingLoans(container,page=0,before=null){const section=el('section','card statement-grid');section.append(el('h2','','Upcoming Loans'));container.insertBefore(section,before);const seq=state.sequence;action(async()=>{const data=await rpc('apiList','UpcomingLoans',{},page,'nextDueDate:asc');if(seq!==state.sequence||!section.isConnected)return;for(const loan of data.rows){const item=button('',()=>openLoan(loan.id),'statement-overview');append(item,el('strong','',loan.nickname),el('strong','',loan.nextRemainingMinor===null?'Amount not set':money(loan.nextRemainingMinor,loan.currency)),el('span','',loan.lender+' · Due '+loan.nextDueDate),el('span',loan.nextDueDate<state.boot.today?'badge overdue':'badge',statementDueLabel(loan.nextDueDate,state.boot.today)));section.append(item);}if(!data.total)section.append(el('p','subtle','No unpaid loan installments.'));if(data.total>40){const nav=el('div','pager'),prev=button('Previous',()=>{renderUpcomingLoans(container,page-1,section);section.remove();}),next=button('Next',()=>{renderUpcomingLoans(container,page+1,section);section.remove();});prev.disabled=!page;next.disabled=(page+1)*40>=data.total;append(nav,prev,next);section.append(nav);}});}
