...
(() => {
  ...
  function simpleTable(container,entity,rows){
    ...
    const wrap=el('div','table-scroll');wrap.tabIndex=0;wrap.setAttribute('aria-label',label(entity)+' table');const table=el('table'),head=el('thead'),tr=el('tr'),columns=state.area==='Overview'&&entity==='Transactions'?['description','cardId','transactionDate','dueDate','amountMinor','currency','type']:(displays[entity]||[]),list=state.entity===entity&&state.area!=='Overview';
