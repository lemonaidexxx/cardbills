...
(() => {
  ...
  function display(r,k){if(k==='type'&&typeGuide[r[k]])return typeGuide[r[k]][0];if(['status','reviewStatus','relationship'].includes(k)&&r[k])return String(r[k]).toLowerCase().replace(/_/g,' ').replace(/^./,c=>c.toUpperCase());if(refs[k])return lookup(refs[k],r[k]);if(/Minor$/.test(k))return money(r[k],r.currency||currencyFor(r));if(r[k]==='')return '—';return String(r[k]??'—');}
  ...
  function openDialog(title,content){dialogReturn=null;if(!$('dialog').open)dialogOpener=document.activeElement;$('dialog-title').textContent=title;$('dialog-content').replaceChildren(content);$('dialog-error').textContent='';if(!$('dialog').open)$('dialog').showModal();$('dialog').scrollTop=0;setTimeout(()=>$('dialog-content').querySelector('input,select,textarea,button')?.focus({preventScroll:true}),0);}
  ...
  let chartEnd='',chartCurrency='',cardMonth='';
  function renderCharts(container){
    ...
    const draw=data=>{if(!section.isConnected||seq!==state.sequence)return;section.replaceChildren();if(data.invalid)return;const currencies=Object.keys(data.currencies);if(!currencies.length)return;if(!currencies.includes(chartCurrency))chartCurrency=currencies[0];const group=data.currencies[chartCurrency],toolbar=el('div','toolbar'),currency=select(currencies.map(id=>({id,label:id})),chartCurrency,false);currency.setAttribute('aria-label','Chart currency');currency.addEventListener('change',()=>{chartCurrency=currency.value;draw(data);});const move=amount=>{chartEnd=new Date(Date.UTC(+requested.slice(0,4),+requested.slice(5,7)-1+amount,1)).toISOString().slice(0,7);section.remove();renderCharts(container);};const next=button('Next year',()=>move(12));next.disabled=requested>=state.boot.today.slice(0,7);append(toolbar,button('Previous year',()=>move(-12)),next,button('Latest 12 months',()=>{chartEnd='';section.remove();renderCharts(container);}),currency);const settings=el('div','chart-settings');settings.setAttribute('popover','auto');settings.append(toolbar);const toggle=button('Chart settings',()=>{const rect=toggle.getBoundingClientRect();settings.style.margin='0';settings.style.inset='auto';settings.style.right=Math.max(16,innerWidth-rect.right)+'px';settings.style.top=Math.min(rect.bottom+8,innerHeight-200)+'px';settings.togglePopover();},'chart-settings-button');toggle.setAttribute('aria-label','Chart settings');toggle.textContent='\u2699';settings.addEventListener('toggle',e=>{toggle.setAttribute('aria-expanded',String(e.newState==='open'));if(e.newState==='closed')toggle.focus({preventScroll:true});else settings.querySelector('button')?.focus();});append(section,el('h2','','Recorded spending by posting month'),toggle,settings);
      ...
      const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');const plotWidth=Math.max(440,Math.round(section.clientWidth-48)),step=(plotWidth-96)/11;svg.setAttribute('viewBox','0 0 '+plotWidth+' 300');svg.setAttribute('role','group');svg.setAttribute('aria-label','Monthly spending trend. Focus or tap a point to see its month and amount.');svg.classList.add('spending-line');const max=Math.max(1,...group.months.map(m=>m.amountMinor)),line=document.createElementNS(svg.namespaceURI,'polyline');line.setAttribute('points',group.months.map((m,i)=>(70+i*step)+','+(240-m.amountMinor/max*200)).join(' '));line.setAttribute('fill','none');line.setAttribute('stroke','#0071E3');line.setAttribute('stroke-width','3');const text=(x,y,value,anchor='middle')=>{const t=document.createElementNS(svg.namespaceURI,'text');t.setAttribute('x',x);t.setAttribute('y',y);t.setAttribute('text-anchor',anchor);t.setAttribute('fill','#717785');t.setAttribute('font-size',plotWidth<600?'14':'12');t.textContent=value;svg.append(t);};for(let i=0;i<=4;i++){const y=240-i*50,axis=document.createElementNS(svg.namespaceURI,'line');axis.setAttribute('x1','70');axis.setAttribute('x2',plotWidth-26);axis.setAttribute('y1',y);axis.setAttribute('y2',y);axis.setAttribute('stroke','#D2D2D7');axis.setAttribute('stroke-opacity',i===0?'1':'.35');svg.append(axis);text(60,y+4,new Intl.NumberFormat(undefined,{notation:'compact',maximumFractionDigits:1}).format(Number(decimal(max*i/4,chartCurrency))),'end');}group.months.forEach((m,i)=>{const date=new Date(m.month+'-01T00:00:00Z');text(70+i*step,265,date.toLocaleString('en',{month:'short',timeZone:'UTC'}));if(i===0||m.month.endsWith('-01'))text(70+i*step,287,m.month.slice(0,4));});svg.append(line);group.months.forEach((m,i)=>{const dot=document.createElementNS(svg.namespaceURI,'circle');dot.setAttribute('cx',70+i*step);dot.setAttribute('cy',240-m.amountMinor/max*200);dot.setAttribute('r','6');dot.setAttribute('fill','#0071e3');dot.setAttribute('tabindex','0');dot.setAttribute('role','link');dot.setAttribute('aria-label',m.month+' '+money(m.amountMinor,chartCurrency));const tip=el('div','chart-tooltip');tip.hidden=true;section.append(tip);const show=()=>{tip.textContent=m.month+' / '+money(m.amountMinor,chartCurrency);tip.hidden=false;};const hide=()=>tip.hidden=true;dot.addEventListener('mouseenter',show);dot.addEventListener('mouseleave',hide);dot.addEventListener('focus',show);dot.addEventListener('blur',hide);dot.addEventListener('click',show);dot.addEventListener('keydown',e=>{if(e.key==='Escape')hide();else if(e.key==='Enter'||e.key===' '){e.preventDefault();show();}});dot.setAttribute('role','button');svg.append(dot);});section.append(svg);      const split=el('div','split');for(const [key,title]of [['currentCards','Spending by card']]){const panel=el('div');panel.append(el('h3','',title),el('p','subtle','Charges less refunds and rebates'));const monthPicker=input(data.currentMonth,'month');monthPicker.max=state.boot.today.slice(0,7);monthPicker.setAttribute('aria-label','Card spending month');monthPicker.addEventListener('change',()=>{if(!monthPicker.value)return;cardMonth=monthPicker.value;section.remove();renderCharts(container);});panel.append(field('Month',monthPicker));const max=Math.max(1,...group[key].map(v=>v.amountMinor));for(const value of group[key].slice().sort((a,b)=>b.amountMinor-a.amountMinor)){const name=key==='currentCards'?(value.id?lookup('Cards',value.id):'No card'):(typeGuide[value.id]?.[0]||value.id),entry=button('',()=>navigate('Transactions',undefined,{...period,spending:'',type:['PURCHASE','FEE','INTEREST','CASH_ADVANCE','INSTALLMENT','REFUND','REBATE'],from:data.currentMonth+'-01',to:new Date(Date.UTC(+data.currentMonth.slice(0,4),+data.currentMonth.slice(5,7),0)).toISOString().slice(0,10),...(key==='currentCards'?(value.id?{cardId:value.id}:{noCard:'true'}):{type:value.id})}),'chart-breakdown');entry.append(el('strong','',name));const totals=el('div','card-spending-totals');for(const [caption,amount]of [['Charges',value.chargesMinor??value.amountMinor],['Refunds and rebates',value.creditsMinor||0],['Net total',value.amountMinor]]){const total=el('span');append(total,el('small','',caption),el('strong','',money(amount,chartCurrency)));totals.append(total);}entry.append(totals);const bar=el('span','chart-bar');bar.style.width=(Math.max(0,value.amountMinor)/max*100)+'%';entry.append(bar);panel.append(entry);}if(!group[key].length)panel.append(el('p','','No recorded spending in this period.'));split.append(panel);}section.append(split);if(group.undatedCount)section.append(button(group.undatedCount+' spending transactions without a posting date',()=>navigate('Transactions',undefined,{currency:chartCurrency,status:'ACTIVE',spending:'true',undated:'true'})));
    };
    if(!chartEnd&&!cardMonth&&state.boot.overview.charts)draw(state.boot.overview.charts);else recordClient('apiList',['OverviewCharts',{endMonth:requested,cardMonth:cardMonth||state.boot.today.slice(0,7)},0,''],()=>seq===state.sequence&&section.isConnected).then(draw).catch(e=>{if(section.isConnected)section.textContent=e.message;});
  }
  ...
  function actionMenu(title,items){
    const wrap=el('div','action-menu'),trigger=button(title,()=>{if(panel.matches(':popover-open'))panel.hidePopover();else{panel.showPopover();const rect=trigger.getBoundingClientRect();panel.style.left=Math.max(12,Math.min(rect.left,innerWidth-panel.offsetWidth-12))+'px';panel.style.top=Math.min(rect.bottom+8,innerHeight-panel.offsetHeight-12)+'px';}}),panel=el('div','action-menu-panel');panel.popover='auto';trigger.setAttribute('aria-expanded','false');trigger.setAttribute('aria-haspopup','true');panel.addEventListener('toggle',e=>{trigger.setAttribute('aria-expanded',String(e.newState==='open'));if(e.newState==='open')panel.querySelector('button')?.focus();});panel.addEventListener('keydown',e=>{if(e.key==='Escape'){panel.hidePopover();trigger.focus();}const buttons=[...panel.querySelectorAll('button')];if(['ArrowDown','ArrowUp'].includes(e.key)){e.preventDefault();buttons[(buttons.indexOf(document.activeElement)+(e.key==='ArrowDown'?1:-1)+buttons.length)%buttons.length]?.focus();}});for(const [name,fn]of items)panel.append(button(name,()=>{panel.hidePopover();trigger.focus();fn();}));append(wrap,trigger,panel);return wrap;
  }
  function renderToolbar(){
    ...
    bar.append(button(state.entity==='Loans'?'Add loan':state.entity==='Transactions'?'Add transaction':'Add '+label(state.entity).toLowerCase(),()=>state.entity==='Loans'?editLoan():editRecord(state.entity)));
    if(state.entity==='Transactions')bar.append(actionMenu('Import',[['Import CSV',showImport],['Import reviewed package',showPackageImport]]));
    if(state.entity==='Shares')bar.append(button('Preview report',showReport));
    if(['Transactions','Shares','Statements','BankPayments','Repayments'].includes(state.entity))bar.append(actionMenu('More',[...(state.entity==='Transactions'?[['Type guide',showTypeGuide]]:[]),['Save this view',()=>editRecord('SavedViews',{name:'',scope:state.entity,filters:JSON.stringify(state.filters),sort:state.sort,status:'ACTIVE'})]]));
  ...
  async function openStatement(id){return action(async()=>{
    const data=await rpc('apiList','Statements',{recordId:id},0,'');const r=data.rows[0];if(!r)throw Error('Statement is unavailable.');
    const box=el('div','loan-details'),grid=el('div','record-detail-grid'),breakdown=el('section','statement-breakdown'),cards=el('div','statement-grid'),cardFilter=el('div','statement-card-filter'),tableBox=el('div'),search=input('');let selected=null,page=0,sort='transactionDate:desc',request=0,scroll=0,focusId='';
    box.append(el('h3','',lookup('Accounts',r.accountId)));const updateSummary=()=>{grid.replaceChildren();for(const [name,value]of [['Statement date',r.statementDate],['Due date',r.dueDate],['Official balance',money(r.balanceMinor,r.currency)],['Allocated payments',money(r.paidMinor,r.currency)],['Remaining balance',money(r.remainingMinor,r.currency)],['Status',statementPaymentLabel(r)]]){const cell=el('div','detail-row');append(cell,el('span','',name),el('strong','',value));grid.append(cell);}};updateSummary();box.append(grid);
    const restore=async()=>{openDialog('Statement details',box);await load();requestAnimationFrame(()=>{$('dialog').scrollTop=scroll;box.querySelector('[data-statement-transaction="'+CSS.escape(focusId)+'"]')?.focus({preventScroll:true});});};
    const back=async()=>{const fresh=await rpc('apiList','Statements',{recordId:id},0,'');if(fresh.rows[0])Object.assign(r,fresh.rows[0]);updateSummary();await restore();};
    const actions=el('div','editor-secondary');append(actions,button('Edit statement',()=>editRecord('Statements',r,back)),button('Record payment',()=>showStatementPayment(r,'FULL',back)),button('Allocate bank payment',()=>editRecord('PaymentAllocations',{statementId:r.id,status:'ACTIVE'},back)));box.append(actions);
    search.setAttribute('aria-label','Search statement transactions');search.placeholder='Search transactions';let debounce;search.addEventListener('input',()=>{page=0;clearTimeout(debounce);debounce=setTimeout(()=>action(load),180);});
    append(breakdown,el('h3','','Transactions by card'),el('p','subtle','Charges less credits excludes financed principal, bank payments and other entries shown separately. These subtotals do not replace the official statement balance.'),cardFilter,cards,field('Search transactions',search),tableBox);box.append(breakdown);
    function transaction(row){scroll=$('dialog').scrollTop;focusId=row.id;const detail=el('div','loan-details'),fields=el('div','record-detail-grid');for(const key of ['description','cardId','transactionDate','postingDate','dueDate','amountMinor','currency','type','reviewStatus','category','tags','notes']){const cell=el('div','detail-row');append(cell,el('span','',label(key)),el('strong','',display(row,key)));fields.append(cell);}append(detail,fields,button('Edit record',()=>editRecord('Transactions',row,back)),button('Back to statement',restore));openDialog('Transaction details',detail);dialogReturn=restore;}
    async function load(){const focusKey=document.activeElement?.dataset.statementSort,sequence=++request,result=await rpc('apiList','StatementBreakdown',{statementId:id,q:search.value,...(selected?selected.cardId?{cardId:selected.cardId,currency:selected.currency}:{noCard:true,currency:selected.currency}:{})},page,sort);if(sequence!==request||!box.isConnected)return;page=result.page;cards.replaceChildren();cardFilter.replaceChildren();const all=button('All cards',()=>{selected=null;page=0;action(load);},'quiet');all.setAttribute('aria-pressed',String(!selected));cardFilter.append(all);for(const group of result.groups){const card=button('',()=>{selected=group;page=0;action(load);},'statement-overview');card.setAttribute('aria-pressed',String(selected?.cardId===group.cardId&&selected?.currency===group.currency));append(card,el('strong','',group.cardId?lookup('Cards',group.cardId):group.nickname),el('span','',group.relationship?group.relationship.charAt(0)+group.relationship.slice(1).toLowerCase():''),el('strong','','Charges less credits: '+money(group.subtotalMinor,group.currency)),el('span','',group.count+' transactions'));if(group.other.length){const otherTotals=el('div','card-other-totals');for(const other of group.other)otherTotals.append(el('span','',typeGuide[other.type]?.[0]||other.type),el('span','',money(other.amountMinor,group.currency)+' · '+other.count+' entries'));card.append(otherTotals);}cards.append(card);}
      const left=tableBox.querySelector('.table-scroll')?.scrollLeft||0;tableBox.replaceChildren();if(!result.total){tableBox.append(el('p','empty','No linked transactions match this view.'));return;}
      const wrap=el('div','table-scroll'),table=el('table'),head=el('thead'),header=el('tr'),body=el('tbody');wrap.tabIndex=0;wrap.setAttribute('aria-label','Statement transactions');for(const key of ['description','cardId','transactionDate','postingDate','amountMinor','currency','type']){const th=el('th'),active=sort.split(':')[0]===key,direction=sort.split(':')[1];th.setAttribute('aria-sort',active?(direction==='asc'?'ascending':'descending'):'none');const sortButton=button(label(key)+(active?(direction==='asc'?' ↑':' ↓'):''),()=>{sort=key+':'+(active&&direction==='asc'?'desc':'asc');page=0;action(load);},'table-sort');sortButton.dataset.statementSort=key;th.append(sortButton);header.append(th);}head.append(header);table.append(head);for(const row of result.rows){const tr=el('tr');for(const key of ['description','cardId','transactionDate','postingDate','amountMinor','currency','type']){const td=el('td',/Minor$/.test(key)?'numeric':'');if(key==='description'){const open=button(row.description,()=>transaction(row),'record-link');open.dataset.statementTransaction=row.id;td.append(open);}else td.textContent=display(row,key);tr.append(td);}body.append(tr);}table.append(body);wrap.append(table);tableBox.append(wrap);wrap.scrollLeft=left;const pager=el('div','pager'),previous=button('Previous',()=>{page--;action(load);}),next=button('Next',()=>{page++;action(load);});previous.disabled=!page;next.disabled=(page+1)*40>=result.total;const controls=el('div');append(controls,previous,next);append(pager,el('span','',result.total+' transactions · page '+(page+1)),controls);tableBox.append(pager);if(focusKey)tableBox.querySelector('[data-statement-sort="'+focusKey+'"]')?.focus({preventScroll:true});
    }
    openDialog('Statement details',box);await load();
  });}
  ...
  function loanCell(cell,row,key){cell.replaceChildren();if(!inlineRows.has(row.id)&&!loanDrafts.has(row.id)){if(key==='nickname')cell.append(button(row.nickname,()=>openLoan(row.id),'record-link'));else cell.textContent=display(row,key);return;}const draft=loanDrafts.get(row.id),value=draft?.values[key]??row[key]??'',control=key==='status'?select(state.boot.enums['Loans.status'],value,false):input(value);control.dataset.loanField=key;control.setAttribute('aria-label',label(key)+' for '+row.nickname);control.disabled=reviewSaving;control.addEventListener('change',()=>{loanSaveFocus={id:row.id,key};const current=loanDrafts.get(row.id)||{row,values:Object.fromEntries(loanFields.map(k=>[k,row[k]||''])),request:uuid()};current.values[key]=control.value;if(loanFields.every(k=>String(current.values[k])===String(row[k]||'')))loanDrafts.delete(row.id);else loanDrafts.set(row.id,current);updateReviewFooter();});cell.append(control);}
  ...
  function renderUpcomingLoans(container,page=0,before=null){const section=el('section','card statement-grid');section.append(el('h2','','Upcoming Loans'));container.insertBefore(section,before);const seq=state.sequence;action(async()=>{const data=await rpc('apiList','UpcomingLoans',{},page,'nextDueDate:asc');if(seq!==state.sequence||!section.isConnected)return;const metrics=$('overview-metrics');if(metrics&&!metrics.querySelector('[data-loan-total]'))for(const [currency,t]of Object.entries(data.loanTotals||{})){const metric=button('',()=>navigate('Loans',undefined,{currency}),'metric');metric.dataset.loanTotal=currency;append(metric,el('span','','Remaining loan payments'),el('strong','',money(t.remaining,currency)),el('span','',t.unknown?t.unknown+' future amounts not set':'Unpaid scheduled payments'));metrics.append(metric);const principal=button('',()=>navigate('Loans',undefined,{currency}),'metric');append(principal,el('span','','Remaining amount without interest'),el('strong','',t.fixedIncluded?money(t.fixedRemainderMinor,currency):'Not available'),el('span','',(t.fixedExcluded||0)+' loans missing original principal'));metrics.append(principal);}for(const loan of data.rows){const item=button('',()=>openLoan(loan.loanId||loan.id),'statement-overview');append(item,el('strong','',loan.nickname),el('strong','',loan.nextRemainingMinor===null?'Amount not set':money(loan.nextRemainingMinor,loan.currency)),el('span','',loan.lender+' · Payment '+loan.installmentNumber+' · Due '+loan.nextDueDate),el('span',loan.nextDueDate<state.boot.today?'badge overdue':'badge',statementDueLabel(loan.nextDueDate,state.boot.today)));section.append(item);}if(!data.total)section.append(el('p','subtle','No unpaid loan installments.'));if(data.total>40){const nav=el('div','pager'),prev=button('Previous',()=>{renderUpcomingLoans(container,page-1,section);section.remove();}),next=button('Next',()=>{renderUpcomingLoans(container,page+1,section);section.remove();});prev.disabled=!page;next.disabled=(page+1)*40>=data.total;append(nav,prev,next);section.append(nav);}});}
  function loanBalanceForm(loan){loanForm('Update lender balance',[['balance','Outstanding principal','text',''],['throughDate','Includes payments through','date',state.boot.today],['reference','Lender reference (optional)','text','']],async(v,request)=>rpc('apiSave','LoanBalance',{loanId:loan.id,balanceMinor:toMinor(v.balance,loan.currency),throughDate:v.throughDate,reference:v.reference},loan._token,request));}
  function loanHistoryForm(loan){loanForm('Payment history completeness',[['complete','All payments since the loan began are recorded','text',loan.principalHistoryComplete||'false',[{id:'false',label:'Not confirmed'},{id:'true',label:'I confirm the payment history is complete'}]]],async(v,request)=>rpc('apiSave','LoanPrincipalHistory',{loanId:loan.id,complete:v.complete},loan._token,request));}
  function loanPaymentPrincipalForm(payment){loanForm('Edit payment principal',[['principal','Principal portion (blank means unknown)','text',payment.principalMinor===''||payment.principalMinor==null?'':decimal(payment.principalMinor,payment.currency)]],async(v,request)=>rpc('apiSave','LoanPaymentPrincipal',{id:payment.id,principalMinor:v.principal===''?'':toMinor(v.principal,payment.currency)},payment._token,request));}
  ...
  function showLoan(loan,data){
    ...
    const actions=el('div','editor-secondary');append(actions,button('Edit loan',()=>nested(()=>editLoan(loan))),button('Set payment period',()=>nested(()=>loanScheduleForm(loan))),button('Record payment',()=>nested(()=>loanPaymentForm(loan))),button('Update lender balance',()=>nested(()=>loanBalanceForm(loan))),button('Payment history completeness',()=>nested(()=>loanHistoryForm(loan))));box.append(actions);const remainder=loan.fixedRemainder||{},calculation=el('section');append(calculation,el('h3','','Remaining amount without interest'),el('p','',remainder.available?money(remainder.remainingMinor,loan.currency):'Original principal needed'),el('p','subtle',remainder.available?money(remainder.principalMinor,loan.currency)+' original principal − '+money(remainder.scheduledMinor,loan.currency)+' known scheduled payments. Includes paid installments; payments are not subtracted twice. This is not a lender-confirmed principal balance.':''));if(remainder.remainingMinor<0)calculation.append(el('p','subtle','Known scheduled payments exceed the original principal.'));box.append(calculation);const principal=loan.principal||{},summary=el('section');append(summary,el('h3','','Remaining loan principal'),el('p','',principal.complete?money(principal.remainingMinor,loan.currency):principal.status||'Balance needed'),el('p','subtle',(principal.basis||'')+(principal.throughDate?' · Includes payments through '+principal.throughDate:'')+(principal.missingCount?' · '+principal.missingCount+' payments need principal details':'')));box.append(summary);
    function table(title,headers,rows){const section=el('section'),wrap=el('div','table-scroll'),t=el('table'),head=el('thead'),tr=el('tr'),body=el('tbody');for(const h of headers)tr.append(el('th','',h));head.append(tr);for(const values of rows){const r=el('tr');for(const [index,value] of values.entries()){const cell=el('td',/amount|principal|scheduled|paid|remaining/i.test(headers[index])&&!/details/.test(headers[index])?'numeric':'');if(value instanceof Node)cell.append(value);else cell.textContent=value;r.append(cell);}body.append(r);}append(t,head,body);wrap.append(t);append(section,el('h3','',title),wrap);if(!rows.length)section.append(el('p','subtle','No records.'));box.append(section);}
    table('Lender balance history',['Includes payments through','Outstanding principal','Reference'],(data.balances||[]).slice().sort((a,b)=>b.throughDate.localeCompare(a.throughDate)).map(b=>[b.throughDate,money(b.balanceMinor,loan.currency),b.reference||'—']));table('Payment periods',['From','Through','Monthly amount','Status','Action'],data.schedules.filter(s=>s.loanId===loan.id).map(s=>[s.startDate,s.endDate,money(s.monthlyMinor,loan.currency),display(s,'status'),button('Edit period',()=>nested(()=>loanScheduleForm(loan,s)))]));
    ...
    table('Payment history',['Date','Amount','Principal portion','Reference','Status','Principal details','Action'],data.payments.filter(p=>p.loanId===loan.id).sort((a,b)=>b.date.localeCompare(a.date)).map(payment=>[payment.date,money(payment.amountMinor,loan.currency),payment.principalMinor===''||payment.principalMinor==null?'Unknown':money(payment.principalMinor,loan.currency),payment.reference||'—',display(payment,'status'),payment.status==='CONFIRMED'?button('Edit principal',()=>nested(()=>loanPaymentPrincipalForm(payment))):'',payment.status==='CONFIRMED'?button('Reverse payment',()=>{const confirmation=el('div'),reversalId=uuid();append(confirmation,el('p','','Reverse '+money(payment.amountMinor,loan.currency)+' paid on '+payment.date+'? Its history will remain.'),button('Cancel',()=>openLoan(loan.id)),button('Confirm reversal',()=>action(async()=>{await rpc('apiSave','ReverseLoanPayment',{id:payment.id},payment._token,reversalId);await refresh();await openLoan(loan.id);})));openDialog('Reverse loan payment',confirmation);dialogReturn=()=>openLoan(loan.id);}):'']));
    loanNestedReturn=null;openDialog('Loan details',box);
  }
  async function renderLoans(){const c=$('content'),seq=state.sequence;state.entity='Loans';return action(async()=>{const data=await rpc('apiList','LoanList',state.filters,state.page,state.sort);if(seq!==state.sequence)return;state.page=data.page;const metrics=el('div','metrics loan-metrics');for(const [currency,t]of Object.entries(data.loanTotals)){const panel=el('section','metric');append(panel,el('span','','Remaining scheduled payments · '+currency),el('strong','',money(t.remaining,currency)),el('span','',t.unknown+' installments need a lender amount'));const overdue=el('section','metric');append(overdue,el('span','','Past due · '+currency),el('strong','',money(t.overdue,currency)),el('span','','Unpaid scheduled amounts'));metrics.append(panel,overdue);}c.append(metrics);renderToolbar();const box=el('div');c.append(box);renderTable(box,data);});}
  ...
  function renderBackups(c){
    const d=state.boot.diagnostics;if(d.automation){const schedule=el('section','card'),a=d.automation;append(schedule,el('h2','','Nightly schedule'),el('p','','Next scheduled run: '+a.nextRun+' · '+a.timezone),el('p','','Calendar: '+(a.calendarEnabled?'Enabled':'Disabled')+' · Last success: '+(a.lastSync||'Not yet completed')),el('p','','Backup: '+(a.backupEnabled?'Enabled':'Disabled')+' · Last success: '+(a.lastBackup||'Not yet completed')));if(a.calendarError)schedule.append(el('p','error',a.calendarError));if(a.backupError)schedule.append(el('p','error',a.backupError));c.append(schedule);}const box=el('section','card'),status=el('p','subtle',d.backupRunning?'Backup in progress…':d.backupError||'Backups do not interrupt your financial workspace.');status.setAttribute('role','status');status.setAttribute('aria-live','polite');
  ...
  async function chooseCalendar(){await action(async()=>{const calendars=await rpc('apiCalendars'),box=el('div'),choice=select(calendars,state.boot.settings.CalendarId),calendarId=input(state.boot.settings.CalendarId||'');calendarId.placeholder='Calendar ID from Google Calendar settings';choice.addEventListener('change',()=>{calendarId.value=choice.value;});append(box,field('Calendar',choice),field('Calendar ID',calendarId),button('Create dedicated calendar',()=>action(async()=>{const x=await rpc('apiCreateCalendar');choice.append(new Option(x.label,x.id));choice.value=x.id;calendarId.value=x.id;notice('Calendar created. Preview selection to use it.');})),button('Preview selection',()=>action(async()=>{const p=await rpc('apiCalendarMigrationPreview',calendarId.value.trim()||choice.value),confirm=el('div');append(confirm,el('p','',p.message),el('p','',p.eventsToRetire+' existing events will have reminders disabled.'),button('Apply calendar selection',()=>action(async()=>{await rpc('apiCalendarMigrate',p.target,p.token,uuid());$('dialog').close();await refresh();})));openDialog('Confirm Calendar migration',confirm);})));openDialog('Select reminder calendar',box);});}
