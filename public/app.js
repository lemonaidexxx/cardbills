...
(() => {
  ...
  async function openStatement(id){return action(async()=>{
    ...
    const box=el('div','loan-details'),grid=el('div','record-detail-grid'),breakdown=el('section','statement-breakdown'),cards=el('div','statement-grid'),tableBox=el('div'),search=input('');let selected=null,page=0,sort='transactionDate:desc',request=0,scroll=0,focusId='';
    ...
    append(breakdown,el('h3','','Transactions by card'),el('p','subtle','Charges less credits excludes financed principal, bank payments and other entries shown separately. These subtotals do not replace the official statement balance.'),cards,field('Search transactions',search),tableBox);box.append(breakdown);
    ...
    async function load(){const focusKey=document.activeElement?.dataset.statementSort,sequence=++request,result=await rpc('apiList','StatementBreakdown',{statementId:id,q:search.value,...(selected?selected.cardId?{cardId:selected.cardId,currency:selected.currency}:{noCard:true,currency:selected.currency}:{})},page,sort);if(sequence!==request||!box.isConnected)return;page=result.page;cards.replaceChildren();const all=button('All cards',()=>{selected=null;page=0;action(load);},'quiet');all.setAttribute('aria-pressed',String(!selected));cards.append(all);for(const group of result.groups){const card=button('',()=>{selected=group;page=0;action(load);},'statement-overview');card.setAttribute('aria-pressed',String(selected?.cardId===group.cardId&&selected?.currency===group.currency));append(card,el('strong','',group.cardId?lookup('Cards',group.cardId):group.nickname),el('span','',group.relationship?group.relationship.charAt(0)+group.relationship.slice(1).toLowerCase():''),el('strong','','Charges less credits: '+money(group.subtotalMinor,group.currency)),el('span','',group.count+' transactions'));for(const other of group.other)card.append(el('span','',typeGuide[other.type]?.[0]||other.type),el('span','',money(other.amountMinor,group.currency)+' · '+other.count+' entries'));cards.append(card);}
      ...
      const wrap=el('div','table-scroll'),table=el('table'),head=el('thead'),header=el('tr'),body=el('tbody');wrap.tabIndex=0;wrap.setAttribute('aria-label','Statement transactions');for(const key of ['description','cardId','transactionDate','postingDate','amountMinor','currency','type']){const th=el('th'),active=sort.split(':')[0]===key,direction=sort.split(':')[1];th.setAttribute('aria-sort',active?(direction==='asc'?'ascending':'descending'):'none');const sortButton=button(label(key)+(active?(direction==='asc'?' ↑':' ↓'):''),()=>{sort=key+':'+(active&&direction==='asc'?'desc':'asc');page=0;action(load);},'table-sort');sortButton.dataset.statementSort=key;th.append(sortButton);header.append(th);}head.append(header);table.append(head);for(const row of result.rows){const tr=el('tr');for(const key of ['description','cardId','transactionDate','postingDate','amountMinor','currency','type']){const td=el('td');if(key==='description'){const open=button(row.description,()=>transaction(row));open.dataset.statementTransaction=row.id;td.append(open);}else td.textContent=display(row,key);tr.append(td);}body.append(tr);}table.append(body);wrap.append(table);tableBox.append(wrap);wrap.scrollLeft=left;const pager=el('div','pager'),previous=button('Previous',()=>{page--;action(load);}),next=button('Next',()=>{page++;action(load);});previous.disabled=!page;next.disabled=(page+1)*40>=result.total;append(pager,el('span','',result.total+' transactions · page '+(page+1)),previous,next);tableBox.append(pager);if(focusKey)tableBox.querySelector('[data-statement-sort="'+focusKey+'"]')?.focus({preventScroll:true});
    }
  ...
  function showLoan(loan,data){
    ...
    table('Lender balance history',['Includes payments through','Outstanding principal','Reference'],(data.balances||[]).slice().sort((a,b)=>b.throughDate.localeCompare(a.throughDate)).map(b=>[b.throughDate,money(b.balanceMinor,loan.currency),b.reference||'—']));table('Payment periods',['From','Through','Monthly amount','Status','Action'],data.schedules.filter(s=>s.loanId===loan.id).map(s=>[s.startDate,s.endDate,money(s.monthlyMinor,loan.currency),s.status,button('Edit period',()=>nested(()=>loanScheduleForm(loan,s)))]));
