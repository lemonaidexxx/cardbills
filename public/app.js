...
(() => {
  ...
  function setupSidebar(){
    const app=document.querySelector('.app'),toggle=$('nav-toggle');if(!toggle)return;
    let collapsed=false;try{collapsed=localStorage.getItem('billsbills-nav-collapsed')==='true';}catch{}
    const apply=()=>{app.classList.toggle('nav-collapsed',collapsed);toggle.replaceChildren(icon(collapsed?'expand':'collapse'));toggle.setAttribute('aria-expanded',String(!collapsed));toggle.setAttribute('aria-label',collapsed?'Expand navigation':'Collapse navigation');toggle.title=collapsed?'Expand navigation':'Collapse navigation';};
    toggle.addEventListener('click',()=>{collapsed=!collapsed;try{localStorage.setItem('billsbills-nav-collapsed',String(collapsed));}catch{}apply();});
    const signout=$('sign-out');signout.replaceChildren(icon('logout'),el('span','nav-label','Sign out'));signout.setAttribute('aria-label','Sign out');signout.title='Sign out';apply();
  }
  ...
  function showStatementPayment(r,initial='FULL'){
    ...
    form.addEventListener('submit',async event=>{event.preventDefault();save.disabled=true;status.textContent='Saving payment information...';try{
      if(!state.boot.workflowRevision)throw Error('Install the BillsBills backend update to use statement payment shortcuts.');
  ...
  const recordClient=createRecordClient(async payload=>{
    ...
    return result.data;
  },{exclusive:fn=>navigator.locks?navigator.locks.request('cardbills-record-request',fn):fn()});
  function rpc(name,...args){return recordClient(name,args);}
  ...
  function renderNav(){$('nav').replaceChildren();Object.keys(areas).forEach(area=>{const name=configured('nav.'+area,area),description=navigationDescriptions[area]||'Manage records and preserve their history.',b=button('',()=>navigate(area),'nav-item');b.replaceChildren(icon(area),el('span','nav-label',name));b.title=name+'\n'+description;b.setAttribute('aria-label',name);b.setAttribute('aria-description',description);if(area===state.area)b.setAttribute('aria-current','page');$('nav').append(b);});}
  ...
  function refresh(){
    ...
    entry.promise=(async()=>{
      ...
      state.boot=b;
      document.querySelector('.brand-title').textContent='BillsBills';document.title='BillsBills';
      $('refresh').textContent=configured('button.Refresh data','Refresh data');
  ...
  async function render(prefetched){if(!state.boot)return;updateReviewFooter();const seq=++state.sequence;$('title').textContent=configured('nav.'+state.area,state.area);$('content').replaceChildren();renderContext();
