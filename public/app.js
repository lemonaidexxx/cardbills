...
(() => {
  ...
  function updateReviewFooter(){
    ...
    bar.hidden=!['Transactions','Review'].includes(state.area)&&!count&&!reviewSaving;
  ...
  async function render(prefetched){if(!state.boot)return;updateReviewFooter();const seq=++state.sequence,view=state.area+'|'+state.entity,sameView=renderedView===view,scroll=$('main').scrollTop,windowScroll=window.scrollY,tableScroll=$('content').querySelector('.table-scroll')?.scrollLeft||0,focusKey=document.activeElement?.dataset.sortKey,focusName=document.activeElement?.name;$('title').textContent=areaName(state.area);
    ...
    if(state.area==='Loans'){renderLoans();return;}
  ...
  function renderOverview(){const o=state.boot.overview,c=$('content');
    ...
    o.upcoming.forEach(s=>{const item=button('',()=>showDetails('Statements',s),'statement-overview');const account=(state.boot.lookups.Accounts||[]).find(a=>a.id===s.accountId);const due=statementDueLabel(s.dueDate,state.boot.today);append(item,el('strong','',account?.label||lookup('Accounts',s.accountId)),el('strong','',money(s.remainingMinor,s.currency)),el('span','','Due '+s.dueDate),el('span',s.dueDate<state.boot.today?'badge overdue':'badge',due));const cards=(state.boot.lookups.Cards||[]).filter(c=>c.accountId===s.accountId);upcoming.append(item);});upcoming.classList.add('statement-grid');c.append(upcoming);
  ...
  function renderToolbar(){
    ...
    if(state.entity==='Transactions'){const edit=button('Edit',()=>{for(const row of state.pageResult?.rows||[])inlineRows.add(row.id);paintInlineRows();updateReviewFooter();});edit.id='edit-page';edit.disabled=reviewSaving||!!reviewJob;edit.classList.add('toolbar-edit');bar.append(edit);}
  ...
  function columnPicker(container,table,entity,columns){
    ...
    const chosen=columnChoices.get(entity),panel=el('div','chart-settings column-settings');panel.setAttribute('popover','auto');
    const apply=()=>table.querySelectorAll('[data-field]').forEach(cell=>cell.hidden=!chosen.has(cell.dataset.field));
    const checks=new Map();
    for(const key of columns){const check=input('','checkbox');check.checked=chosen.has(key);check.disabled=key===columns[0];check.addEventListener('change',()=>{check.checked?chosen.add(key):chosen.delete(key);apply();});checks.set(key,check);panel.append(append(el('label','check-label'),check,document.createTextNode(label(key))));}
  ...
  function editRecord(entity,record={},onReturn){
    ...
    if(entity==='Transactions')transactionPlanPicker(grid,controls,record);
    ...
    form.addEventListener('submit',async e=>{e.preventDefault();save.disabled=true;$('dialog-error').textContent='';try{
      ...
      const result=await rpc('apiSave',entity,data,record._token||'',requestId);$('dialog').close();await refresh();if(onReturn)onReturn(result.id);notice('Record saved.');
  ...
  function transactionPlanPicker(grid,controls,record){
    ...
    const section=el('section','wide installment-editor'),search=input(),results=el('div'),summary=el('p','subtle');
    search.setAttribute('aria-label','Search installment plans');search.placeholder='Search plan reference or ID';
    const sequenceLabel=field('Payment sequence',sequence);sequence.type='number';sequence.min='1';
    const compatible=p=>p.accountId===controls.accountId.value&&p.currency===controls.currency.value&&(!p.cardId||p.cardId===controls.cardId.value);
    const plans=()=>state.boot.lookups.InstallmentPlans||[];
    function update(){const selected=plans().find(p=>p.id===plan.value);summary.textContent=selected?'Number of payments: '+selected.count+(sequence.value?' · '+sequence.value+' of '+selected.count:''):'No installment plan selected';sequence.max=selected?String(selected.count):'';sequence.disabled=controls.type.value!=='INSTALLMENT';sequenceLabel.hidden=sequence.disabled;}
    function draw(){results.replaceChildren();const q=search.value.trim().toLowerCase();const matches=plans().filter(p=>compatible(p)&&p.status!=='ARCHIVED'&&(!q||(p.label+' '+p.id).toLowerCase().includes(q)));for(const p of matches)results.append(button(p.label+' — '+p.count+' payments',()=>{plan.value=p.id;update();}));if(!matches.length)results.append(el('p','subtle','No matching plans.'));}
    function returnToTransaction(id){const draft={...record};for(const [k,n]of Object.entries(controls))draft[k]=/Minor$/.test(k)?toMinor(n.value,controls.currency.value):n.value;if(id&&draft.type==='INSTALLMENT')draft.installmentPlanId=id;editRecord('Transactions',draft);}
    const create=button('Create plan',()=>editRecord('InstallmentPlans',{accountId:controls.accountId.value,cardId:controls.cardId.value,currency:controls.currency.value,startDate:controls.transactionDate.value,reference:controls.description.value,...(controls.type.value==='FINANCED_PRINCIPAL'?{originTransactionId:record.id}:{})},returnToTransaction));
    const edit=button('Edit plan',()=>action(async()=>{if(!plan.value)throw new Error('Choose a plan first.');const response=await rpc('apiList','InstallmentPlans',{recordId:plan.value},0,'');if(!response.rows[0])throw new Error('Plan is unavailable.');editRecord('InstallmentPlans',response.rows[0],returnToTransaction);}));
    search.addEventListener('input',draw);sequence.addEventListener('input',update);
    for(const key of ['accountId','cardId','currency'])controls[key].addEventListener('change',()=>{const selected=plans().find(p=>p.id===plan.value);if(selected&&!compatible(selected)){plan.value='';sequence.value='';}draw();update();});
    controls.type.addEventListener('change',update);
    append(section,el('h3','','Installment plan'),field('Installment Plan ID',search),results,summary,sequenceLabel,button('Clear plan',()=>{plan.value='';sequence.value='';update();}),create,edit,el('p','subtle','The total number of payments belongs to the shared plan. Editing it affects all linked charges. Financed principal is linked through the plan’s principal field.'));
    grid.append(section);draw();update();
  }
  function loanForm(title,fields,onSave){const form=el('form'),grid=el('div','form-grid'),controls={};for(const [key,labelText,type,value,choices]of fields){const control=choices?select(choices,value,false):input(value??'',type||'text');control.setAttribute('aria-label',labelText);controls[key]=control;const labelNode=el('label','',labelText);labelNode.append(control);grid.append(labelNode);}if(controls.firstDueDate&&controls.dueDay)controls.firstDueDate.addEventListener('change',()=>{controls.dueDay.value=Number(controls.firstDueDate.value.slice(8))||1;});const save=button('Save',()=>{},'primary');save.type='submit';append(form,grid,save);const request=uuid();form.addEventListener('submit',async e=>{e.preventDefault();save.disabled=true;try{await onSave(Object.fromEntries(Object.entries(controls).map(([k,v])=>[k,v.value])),request);$('dialog').close();await refresh();}catch(error){$('dialog-error').textContent=error.message;}finally{save.disabled=false;}});openDialog(title,form);}
  ...
  function loanPaymentForm(loan,installment){const box=el('div'),date=input(state.boot.today,'date'),amount=input(installment?decimal(installment.remainingMinor,loan.currency):''),reference=input(''),notes=input(''),allocationRows=el('div'),entries=[];date.setAttribute('aria-label','Actual payment date');amount.setAttribute('aria-label','Payment amount');append(box,field('Actual payment date',date),field('Payment amount',amount),field('Reference',reference),field('Notes',notes),el('p','subtle','Allocate the full payment. Past dates and partial payments are supported. Nothing is recorded until confirmation.'),allocationRows);const add=(selected)=>{const row=el('div','share-line'),choice=select(loan.installments.filter(i=>i.remainingMinor>0).map(i=>({id:String(i.number),label:'#'+i.number+' / '+i.dueDate+' / '+money(i.remainingMinor,loan.currency)})),selected?String(selected.number):'',true),value=input(selected?decimal(selected.remainingMinor,loan.currency):'');choice.setAttribute('aria-label','Installment allocation');value.setAttribute('aria-label','Allocated amount');const entry={row,choice,value};entries.push(entry);append(row,choice,value,button('Remove',()=>{entries.splice(entries.indexOf(entry),1);row.remove();}));allocationRows.append(row);};add(installment);box.append(button('Allocate another installment',()=>add()));const request=uuid();box.append(button('Review payment',()=>{try{const payload={loanId:loan.id,date:date.value,amountMinor:toMinor(amount.value,loan.currency),reference:reference.value,notes:notes.value,allocations:entries.map(e=>({number:Number(e.choice.value),amountMinor:toMinor(e.value.value,loan.currency)}))};if(payload.allocations.reduce((sum,a)=>sum+a.amountMinor,0)!==payload.amountMinor)throw Error('Allocations must equal the payment amount.');const confirmBox=el('div');append(confirmBox,el('p','',loan.nickname+' / '+money(payload.amountMinor,loan.currency)+' paid on '+payload.date),el('p','',payload.allocations.map(a=>'Installment '+a.number+': '+money(a.amountMinor,loan.currency)).join('; ')),button('Back',()=>openDialog('Record loan payment',box)),button('Confirm payment',()=>action(async()=>{await rpc('apiSave','LoanPayment',payload,loan._token,request);$('dialog').close();await refresh();}),'primary'));openDialog('Confirm loan payment',confirmBox);}catch(e){$('dialog-error').textContent=e.message;}},'primary'));openDialog('Record loan payment',box);}
  function showLoan(loan,data){const box=el('div');append(box,el('h3','',loan.nickname),el('p','',loan.lender+' / '+loan.currency),el('p','subtle','Scheduled amounts are not an outstanding-principal calculation.'),button('Edit loan',()=>editLoan(loan)),button('Set payment period',()=>loanScheduleForm(loan)),button('Record payment',()=>loanPaymentForm(loan)));const periods=el('section');periods.append(el('h3','','Payment periods'));for(const segment of data.schedules.filter(s=>s.loanId===loan.id))periods.append(button(segment.startDate+' to '+segment.endDate+' / '+money(segment.monthlyMinor,loan.currency)+' / '+segment.status,()=>loanScheduleForm(loan,segment)));box.append(periods);const schedule=el('div','loan-schedule');for(const i of loan.installments){const row=el('div','toolbar');append(row,el('span','','#'+i.number+' / '+i.dueDate),el('strong','',i.amountMinor===null?'Amount not set':money(i.amountMinor,loan.currency)),el('span','',i.status==='Upcoming'?statementDueLabel(i.dueDate,state.boot.today):i.status),el('span','',i.paidMinor?'Paid '+money(i.paidMinor,loan.currency):''));if(i.remainingMinor>0)row.append(button('Record payment / Mark paid',()=>loanPaymentForm(loan,i)));schedule.append(row);}append(box,el('h3','','Installments'),schedule,el('h3','','Payment history'));for(const payment of data.payments.filter(p=>p.loanId===loan.id).sort((a,b)=>b.date.localeCompare(a.date))){const row=el('div','toolbar');append(row,el('span','',payment.date+' / '+money(payment.amountMinor,loan.currency)+' / '+payment.status+' / '+payment.reference));if(payment.status==='CONFIRMED')row.append(button('Reverse payment',()=>{const confirmation=el('div'),reversalId=uuid();append(confirmation,el('p','','Reverse '+money(payment.amountMinor,loan.currency)+' paid on '+payment.date+'? Its history will remain.'),button('Cancel',()=>showLoan(loan,data)),button('Confirm reversal',()=>action(async()=>{await rpc('apiSave','ReverseLoanPayment',{id:payment.id},payment._token,reversalId);$('dialog').close();await refresh();})));openDialog('Reverse loan payment',confirmation);}));box.append(row);}openDialog('Loan details',box);}
  function renderLoans(){const c=$('content'),seq=state.sequence;c.append(button('Add loan',()=>editLoan()));action(async()=>{const data=await rpc('apiList','LoanDashboard',{},0,'');if(seq!==state.sequence)return;const totals={};for(const loan of data.loans.filter(l=>l.status==='ACTIVE')){const t=totals[loan.currency]||(totals[loan.currency]={remaining:0,overdue:0,unknown:0});for(const i of loan.installments){if(i.remainingMinor===null)t.unknown++;else{t.remaining+=i.remainingMinor;if(i.dueDate<state.boot.today)t.overdue+=i.remainingMinor;}}}const metrics=el('div','metrics loan-metrics');for(const [currency,t]of Object.entries(totals)){const panel=el('section','metric');append(panel,el('h3','',currency+' loan schedule'),el('p','','Known remaining scheduled payments: '+money(t.remaining,currency)),el('p','','Past due: '+money(t.overdue,currency)),el('p','subtle',t.unknown+' installments need a lender amount'));metrics.append(panel);}c.append(metrics);const upcoming=el('section','card loan-upcoming');upcoming.append(el('h2','','Upcoming and overdue loan payments'));const due=data.loans.filter(l=>l.status==='ACTIVE').flatMap(loan=>loan.installments.filter(i=>i.remainingMinor===null||i.remainingMinor>0).map(i=>({loan,i}))).sort((a,b)=>a.i.dueDate.localeCompare(b.i.dueDate));for(const {loan,i}of due.slice(0,12))upcoming.append(button(loan.nickname+' / '+i.dueDate+' / '+(i.remainingMinor===null?'Amount not set':money(i.remainingMinor,loan.currency))+' / '+(i.status==='Upcoming'?statementDueLabel(i.dueDate,state.boot.today):i.status),()=>showLoan(loan,data)));if(!due.length)upcoming.append(el('p','','No outstanding scheduled payments.'));c.append(upcoming);const grid=el('div','loan-grid');for(const loan of data.loans){const card=el('section','card');append(card,el('h3','',loan.nickname),el('p','',loan.lender),el('p','',loan.termMonths+' months / '+loan.status),button('Open loan',()=>showLoan(loan,data)));grid.append(card);}c.append(grid);});}
  function renderReview(){const c=$('content');renderDuplicateReview(c);const section=el('section','card');append(section,el('h2','','Transactions needing a type'),el('p','','Choose the transaction type, then save changes here.'));c.append(section);state.entity='Transactions';state.filters={type:'UNKNOWN'};const seq=state.sequence;action(async()=>{const result=await rpc('apiList','Transactions',state.filters,state.page,state.sort||'transactionDate:desc');if(seq!==state.sequence)return;state.pageResult=result;const edit=button('Edit',()=>{result.rows.forEach(r=>inlineRows.add(r.id));paintInlineRows();updateReviewFooter();});edit.id='edit-page';section.append(edit);renderTable(section,result);});const list=el('div','issues');for(const issue of state.boot.issues){const row=el('section','card');append(row,el('h3','',label(issue.entity)),el('p','',issue.message));if(issue.message.startsWith('A tracked row was removed')){for(const mode of ['restore','accept'])row.append(button(mode==='restore'?'Restore record':'Accept intentional deletion',()=>action(async()=>{await rpc('apiResolveMissing',issue.entity,issue.id,mode,uuid());await refresh();})));}else if(issue.entity==='Operations')row.append(button('Resume operation',()=>action(async()=>{await rpc('apiRecover',issue.id);await refresh();})));else if(issue.entity==='Settings'&&!issue.id)row.append(button('Repair missing settings',()=>action(async()=>{await rpc('repairSettings');await refresh();})));else row.append(button('Fix record',()=>action(async()=>{const data=await rpc('apiList',issue.entity,{recordId:issue.id},0,'updatedAt:desc');if(!data.rows[0])throw Error('Record unavailable. Restore its linked record or resolve the pending operation first.');editRecord(issue.entity,data.rows[0]);})));list.append(row);}c.append(list);}
  function shareEditor(transaction){action(async()=>{const data=await rpc('apiList','Shares',{transactionId:transaction.id,status:'ACTIVE'},0,'updatedAt:asc');if(data.total>30)throw Error('This transaction has more than 30 shares; edit individual shares in Money owed.');const box=el('div'),rows=el('div'),message=el('p','subtle'),items=[];const add=share=>{const line=el('div','share-line'),person=select(state.boot.lookups.People||[],share.personId||''),amount=input(share.amountMinor===undefined?'':decimal(share.amountMinor,transaction.currency));person.setAttribute('aria-label','Person');amount.setAttribute('aria-label','Share amount');append(line,field('personId',person),field('Amount',amount));if(!share.id)line.append(button('Remove',()=>{line.remove();items.splice(items.findIndex(i=>i.line===line),1);}));items.push({share,person,amount,line});rows.append(line);};data.rows.forEach(add);if(!data.rows.length)add({});const total=input(decimal(transaction.amountMinor,transaction.currency));total.setAttribute('aria-label','Total to split');append(box,el('h3','',transaction.description),field('Total to split',total),rows,button('Add person',()=>add({})),button('Split equally',()=>{try{const sum=toMinor(total.value,transaction.currency);if(!items.length||sum<=0)throw Error('Enter a positive total.');const each=Math.floor(sum/items.length),remainder=sum-each*items.length;items.forEach((item,i)=>item.amount.value=decimal(each+(i<remainder?1:0),transaction.currency));message.textContent=remainder?'Rounding: the first '+remainder+' shares receive one extra minor unit.':'Equal amounts; no rounding remainder.';}catch(e){message.textContent=e.message;}}),message);const request=uuid(),save=button('Save shares',()=>action(async()=>{save.disabled=true;try{await rpc('apiSave','ShareBatch',{transactionId:transaction.id,items:items.map(i=>({id:i.share.id,token:i.share._token,personId:i.person.value,amountMinor:toMinor(i.amount.value,transaction.currency)}))},transaction._token,request);$('dialog').close();await refresh();}finally{save.disabled=false;}}),'primary');box.append(save);openDialog('Assign shares',box);});}
