...
(() => {
  ...
  function renderOverview(){const o=state.boot.overview,c=$('content');
    ...
    o.upcoming.forEach(s=>{const item=button('',()=>showDetails('Statements',s),'statement-overview');const account=(state.boot.lookups.Accounts||[]).find(a=>a.id===s.accountId);const due=statementDueLabel(s.dueDate,state.boot.today);append(item,el('strong','',account?.label||lookup('Accounts',s.accountId)),el('strong','',money(s.remainingMinor,s.currency)),el('span','','Due '+s.dueDate),el('span',s.dueDate<state.boot.today?'badge overdue':'badge',due));const cards=(state.boot.lookups.Cards||[]).filter(c=>c.accountId===s.accountId);if(cards.length)item.append(el('span','statement-cards',cards.map(c=>c.label).join(', ')));upcoming.append(item);});upcoming.classList.add('statement-grid');c.append(upcoming);
const intro=el('section','overview-intro');append(intro,el('h2','','Your finances, clearly organized.'));c.append(intro);
  ...
  function renderCharts(container){
    ...
    const draw=data=>{if(!section.isConnected||seq!==state.sequence)return;section.replaceChildren();if(data.invalid)return;const currencies=Object.keys(data.currencies);if(!currencies.length)return;if(!currencies.includes(chartCurrency))chartCurrency=currencies[0];const group=data.currencies[chartCurrency],toolbar=el('div','toolbar'),currency=select(currencies.map(id=>({id,label:id})),chartCurrency,false);currency.setAttribute('aria-label','Chart currency');currency.addEventListener('change',()=>{chartCurrency=currency.value;draw(data);});const move=amount=>{chartEnd=new Date(Date.UTC(+requested.slice(0,4),+requested.slice(5,7)-1+amount,1)).toISOString().slice(0,7);section.remove();renderCharts(container);};const next=button('Next year',()=>move(12));next.disabled=requested>=state.boot.today.slice(0,7);append(toolbar,button('Previous year',()=>move(-12)),next,button('Latest 12 months',()=>{chartEnd='';section.remove();renderCharts(container);}),currency);append(section,el('h2','','Recorded spending by posting month'),toolbar,el('p','subtle',group.months[0].month+' to '+data.endMonth));
      ...
      const svg=document.createElementNS('http://www.w3.org/2000/svg','svg');svg.setAttribute('viewBox','0 0 720 220');svg.setAttribute('role','img');svg.setAttribute('aria-label','Monthly spending trend. Exact amounts and month links follow.');svg.classList.add('spending-line');const max=Math.max(1,...group.months.map(m=>m.amountMinor)),line=document.createElementNS(svg.namespaceURI,'polyline');line.setAttribute('points',group.months.map((m,i)=>(30+i*60)+','+(190-m.amountMinor/max*160)).join(' '));line.setAttribute('fill','none');line.setAttribute('stroke','#0071E3');line.setAttribute('stroke-width','3');svg.append(line);group.months.forEach((m,i)=>{const dot=document.createElementNS(svg.namespaceURI,'circle');dot.setAttribute('cx',30+i*60);dot.setAttribute('cy',190-m.amountMinor/max*160);dot.setAttribute('r','5');dot.setAttribute('fill','#0071e3');dot.setAttribute('tabindex','0');dot.setAttribute('role','link');dot.setAttribute('aria-label',m.month+' '+money(m.amountMinor,chartCurrency));const open=()=>navigate('Transactions',undefined,{...period,from:m.month+'-01',to:new Date(Date.UTC(+m.month.slice(0,4),+m.month.slice(5,7),0)).toISOString().slice(0,10)});dot.addEventListener('click',open);dot.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();open();}});svg.append(dot);const text=document.createElementNS(svg.namespaceURI,'text');text.setAttribute('x',30+i*60);text.setAttribute('y','215');text.setAttribute('text-anchor','middle');text.setAttribute('font-size','10');text.textContent=m.month.slice(5);svg.append(text);});section.append(svg);const points=el('div','chart-months');group.months.forEach(m=>points.append(button(m.month+' '+money(m.amountMinor,chartCurrency),()=>navigate('Transactions',undefined,{...period,from:m.month+'-01',to:new Date(Date.UTC(+m.month.slice(0,4),+m.month.slice(5,7),0)).toISOString().slice(0,10)}),'quiet')));section.append(points);
      const split=el('div','split');for(const [key,title]of [['cards','Spending by card'],['types','Spending by transaction type']]){const panel=el('div');panel.append(el('h3','',title));const max=Math.max(1,...group[key].map(v=>v.amountMinor));for(const value of group[key].slice().sort((a,b)=>b.amountMinor-a.amountMinor)){const name=key==='cards'?(value.id?lookup('Cards',value.id):'No card'):(typeGuide[value.id]?.[0]||value.id),entry=button(name+' '+money(value.amountMinor,chartCurrency),()=>navigate('Transactions',undefined,{...period,...(key==='cards'?(value.id?{cardId:value.id}:{noCard:'true'}):{type:value.id})}),'chart-breakdown');const bar=el('span','chart-bar');bar.style.width=(value.amountMinor/max*100)+'%';entry.append(bar);panel.append(entry);}if(!group[key].length)panel.append(el('p','','No recorded spending in this period.'));split.append(panel);}section.append(split);if(group.undatedCount)section.append(button(group.undatedCount+' spending transactions without a posting date',()=>navigate('Transactions',undefined,{currency:chartCurrency,status:'ACTIVE',spending:'true',undated:'true'})));
    };
  ...
  function renderReview(){const c=$('content');renderDuplicateReview(c);append(c,el('p','subtle','Errors block totals, imports, reports and synchronization. Warnings identify records requiring judgment. Use Diagnostics and recovery in Settings to resolve missing references before continuing.'));
    if(!state.boot.issues.length){c.append(el('div','empty','No other review issues found.'));return;}const list=el('div','issues');state.boot.issues.forEach(x=>{const row=el('div','issue '+(x.severity==='ERROR'?'error':''));append(row,el('strong','',label(x.entity)),el('p','',x.message));if(x.message.startsWith('A tracked row was removed')){
