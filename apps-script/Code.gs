...
function token_(r) { const c={}; Object.keys(r).sort().filter(k=>!k.startsWith('_')).forEach(k=>c[k]=r[k]); return hash_(JSON.stringify(c)); }
...
function reconcileSheetEdits_() {
  ...
  Object.keys(CC_SCHEMA).filter(e=>!CC_INTERNAL.includes(e)).forEach(entity=>db[entity].forEach(row=>{
    ...
    if(entity==='Statements'&&props_().getProperty('CC_WRITE_'+row.id))return;
    ...
    if(old&&token_(old)===token_(row)){if(old._acceptedDeletion)saveBaseline_(entity,row);return;}
    try{
...
function decorate_(e,r,db) {
  ...
  const out=Object.assign({},r,{_token:token_(r)});
